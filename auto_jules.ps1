...
function Invoke-JulesForRange {
    ...
    $body = @{
        ...
        sourceContext       = @{
            source            = "sources/github/banana-hime/temp_make_wha_is_me"
    ...
    if (-not $prUrl) {
        ...
        $prUrl = gh pr list --repo "banana-hime/temp_make_wha_is_me" --limit 1 --json url --jq ".[0].url"
    }

    Write-Host "🛠️ PR 承認とマージを実行します: $prUrl" -ForegroundColor Cyan
    gh pr edit $prUrl --add-assignee "banana-hime"

    # --- Verification Step Skipped ---
    Write-Host "⚠️ 整合性チェックをスキップします。PRの内容を正としてマージします。" -ForegroundColor Yellow
    # --- End Verification Step ---

    gh pr review $prUrl --approve --body "Approved by banana-hime automation script."
